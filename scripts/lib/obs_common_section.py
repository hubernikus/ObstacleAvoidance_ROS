...
def obs_common_section(obs):
        ...
        for it_obs2 in range(it_obs1+1,N_obs):
            ...
            if intersection_with_obs1:# Modify intersecition part
                ...
                if sqrt(sum((np.array(obs[it_obs1].x0)-np.array(obs[it_obs2].x0))**2)) < max(obs[it_obs1].a) + max(obs[it_obs2].a): # Obstacles are close enough
                    ...
                    Gamma_temp = (rotMat[:,:,it_obs1].T.dot(np.array(obs[it_obs2].x_obs_sf).T-np.tile(obs[it_obs1].x0,(N_points,1)).T ) / np.tile(obs[it_obs1].a, (N_points,1)).T )
                    Gamma = np.sum( (1/obs[it_obs1].sf *  Gamma_temp) ** (2*np.tile(obs[it_obs1].p, (N_points,1)).T), axis=0)
                    print('t obs', it_obs2)
                    print('sf', np.array(obs[it_obs1].x_obs_sf))
                    print('sf', np.array(obs[it_obs2].x_obs_sf))
                    intersection_sf_temp = np.array(obs[it_obs2].x_obs_sf)[Gamma<1,:].T

                    # Get all poinst of obs1 in obs2
                    #                 R = compute_R(d,obs[it_obs2].th_r)
                    Gamma_temp = ( rotMat[:,:,it_obs2].T.dot(np.array(obs[it_obs1].x_obs_sf).T-np.tile(obs[it_obs2].x0,(N_points,1)).T ) / np.tile(obs[it_obs2].a, (N_points,1)).T )
    ...
    for ii in range(len(intersection_obs)):
        ...
        # TODO -- unique .... (to avoid repetitions)
        #intersection_sf[ii] = np.unique(intersection_sf[ii], axis=1)
        #intersection_sf[ii] = np.unique(intersection_sf[ii].T).T

        # Get numerical mean
        x_center_dyn = np.zeros((d))
        for dd in range(d):
            x_center_dyn[dd] = np.mean(intersection_sf[ii][dd,:])
